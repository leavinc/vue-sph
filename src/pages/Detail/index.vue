          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr) in spuSaleAttrList" :key="attr.id">
                ...
                <dd changepirce="0" v-for="(attrValue) in attr.spuSaleAttrValueList" :key="attrValue.id" :class="{active:attrValue.isChecked === '1'}" @click = 'changeActive(attrValue,attr.spuSaleAttrValueList)'>{{attrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeSkuNum(event){
        // 如果用户输入非法
        if(isNaN(event.value) || event.value <1){
          this.skuNum = 1
        }else {
          this.skuNum = parseInt(event.value)
        }
      },
      // 加入购物车
      async addShopCart(){
        // 发请求，将产品信息传递给数据库
        try {
          // 这里的skuNum传递的是选择的商品数量，后台服务器中商品的属性skuNum初始值为0，我们传递过去的值，会加到其初始值上，这样就可以得到我们需要的数量
           await this.$store.dispatch('detail/addOrUpdate',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
           alert('加入购物车成功~~~')
          //  会话存储,只能存储字符串
